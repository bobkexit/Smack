...
class ChatVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    @IBAction func messageTxtEditing(_ sender: Any) {
        ...
        if messageTxt.text == "" {
            isTyping = false
            sendBtn.isHidden = true
            SocketService.instance.manager.defaultSocket.emit(SOCKET_EVT_STOP_TYPE, UserDataService.instance.name, channelId)
        } else {
            if !isTyping {
                sendBtn.isHidden = false
                SocketService.instance.manager.defaultSocket.emit(SOCKET_EVT_START_TYPE, UserDataService.instance.name, channelId)
            }
            isTyping = true
        }
    }
